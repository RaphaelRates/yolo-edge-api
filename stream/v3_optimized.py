...
class OptimizedCamera:
    ...
    def start(self):
        try:
            ...
            print(f"[ERROR] Falha ao iniciar câmera: {e}")
            raise
    ...
    def _loop(self):
        while self._running.is_set():
            try:
                ...
                print(f"[ERROR] Erro no loop da câmera: {e}")
                break
...
class RealtimeDetector:
    ...
    def __init__(self, model_path: str, conf: float,
        ...
        if not Path(model_path).exists():
            print(f"[WARNING] Modelo '{model_path}' não encontrado. Verifique o caminho.")
...
def check_display_support():
    """Verifica se o OpenCV tem suporte a display gráfico"""
    try:
        # Tenta criar e destruir uma janela mínima
...
def main():
    ...
    # Verifica suporte a display
    display_enabled = not args.no_display and check_display_support()
    if args.no_display:
        print("[INFO] Modo headless ativado (--no-display)")
    elif not display_enabled:
        print("[INFO] OpenCV sem suporte gráfico. Executando headless.")
    ...
    if args.output:
        ...
        if not writer.isOpened():
            print(f"[ERROR] Falha ao abrir arquivo de saída: {args.output}")
            return
    ...
    # Nome da janela consistente
    window_name = "YOLO — Tempo Real (pressione q para sair)"
    ...
    try:
        ...
        print(f"[INFO] Frames capturados: {camera.frames_in}")
        print(f"[INFO] Frames entregues: {camera.frames_out}")
